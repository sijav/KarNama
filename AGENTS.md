...
- eslint-plugin-react-hooks 7.1.1's recommended rules, which apps/web turns on, include `purity`, `set-state-in-render` and `set-state-in-effect`: calling `Date.now()` while a component renders is an error, where the same read inside a snapshot handed to `useSyncExternalStore`, or inside a timer, lints clean. A design can be linted before it is written: ESLint's `lintText`, given the file's own path, lints text in memory with the workspace's rules, types included, and `calculateConfigForFile` lists the rules the file gets (KN-325's plan).
