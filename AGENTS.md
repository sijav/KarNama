...
- A formatter run OUTSIDE the repository is a DIFFERENT formatter, so a drift number measured on a copy is a measurement of the wrong thing. Copying a file to a temporary directory and running `prettier --write` there loses the project's configuration: it falls back to double quotes, semicolons and an eighty column wrap, and reports most of the file as drift. That method said `App.tsx` had gone from a baseline of 120 to 142, and the next move it invited was cutting comments out of a correct change to bring a phantom overrun down. `prettier --stdin-filepath <the file's real path>` resolves the configuration from that path and says 0. What hid it was corroboration: `DESIGN.md` measures 236 under both methods, matching the baseline KN-468 recorded, so the broken numbers looked confirmed by a known-good one (KN-473).
- **Behind Cloudflare, `X-Forwarded-For` is caller controlled and `CF-Connecting-IP` is not.** Cloudflare OVERWRITES `CF-Connecting-IP` on every request, so its value is outside the caller's reach; it only APPENDS to `X-Forwarded-For`, so a caller who sends their own copy owns the leftmost entry and can claim any address. A rate limit, an allowlist or an abuse trail keyed on the latter is one the caller chooses, which is WORSE than one bucket for everybody: it looks fixed and is trivially evaded. Express's numeric `trust proxy` reads that same header from the right, so it inherits the problem, and Render publishes no chain length to count anyway. Render documents the answer on its own site and Cloudflare's header reference agrees. KN-484, where the first plan asserted that trusting one hop would return Cloudflare's address; it would not, and what it returns is not deducible without knowing the header that arrives.
...
- A regex comment stripper cannot prove a change is comments only: it is not a lexer, so it also erases text inside a string, a template literal or a regex literal holding `/*` or `//`, and a code change made inside one vanishes from BOTH copies and compares equal. `git diff` does not settle it either, being line based and equally blind to lexical context, so a changed line inside a template literal reads like any other. What settles it is reading the changed lines yourself and judging what each one is. KN-708 claimed the stripper sound and KN-710 corrected the claim; neither of them is a gate (KN-710).
- **Playwright's `clock.install()` does NOT stop time.** It replaces `Date`, `setTimeout`, `setInterval`, the animation-frame and idle callbacks and `performance` with fake ones, and those go on FLOWING. Only `pauseAt`, or `fastForward` which jumps forward and pauses, stops them, and `resume()` exists to start them again. Playwright's own `pauseAt` example spells out the intended order: install at a fixed time, "let the page load naturally", because "Date.now will progress as the timers fire", and only then pause. KN-695's clocked spec installed and typed straight away, so more than the 300 ms debounce elapsed of its own accord while the assertions ran, and all four runs failed at exactly the assertion that came after enough time, **mutated and restored alike**, which is a control that cannot tell the two apart and therefore proves nothing. The order that works is install, seed while time runs, `pauseAt`, then `runFor` to step it by hand. Read this from `node_modules/playwright-core/types/types.d.ts` rather than from memory: several paragraphs of my reasoning about when the timer was scheduled were answered by one line of its own documentation, and the repository's rule is already to verify versions and behaviour from source.
- **Design a mutation by what it LEAVES BEHIND, not by what it means.** This section already said that the e2e webServer runs `tsc --noEmit && vite build` before anything, so a mutation leaving a symbol unused stops the server rather than failing a test, and KN-695's control walked into it regardless: putting the filtering read back on the typed value orphaned the applied one, `noUnusedLocals` refused, the server never started, and Playwright ran NO test. Knowing the rule and applying it while writing the mutation are separate acts, so the check has to be a step: before running a control, name every symbol the mutation stops using and every file that must still compile. The rewrite made one handler set both values, which is the same old behaviour with nothing orphaned. And the failure mode is the dangerous one, because "could not run" arrives wearing the clothes of "does not hold": a control that cannot distinguish them will report the defect as proved when nothing was exercised at all.
- **Two guards in sequence can make each other untestable, and a fixture that cannot tell them apart does not prove the policy.** KN-716's shell returns early on `if (!latch) return` and again on `if (focus is not the body) return`. In the story written to prove it, the latch was false AND focus was on the control, so EITHER guard alone was enough: removing the first still stopped at the second, removing the second still stopped at the first, and NEITHER removal failed anything. Only removing both did, which proves a narrowing exists without proving which condition draws the line, and the line was the whole card. The fix is a fixture that satisfies one guard and not the other, so each becomes individually load bearing: here, an ordinary navigation where focus IS lost, which leaves only the latch holding the shell back. Work the planted failure out BEFORE writing the stories, because it is what tells you which fixtures you actually need. And check that the way you DRIVE the fixture does not destroy the condition it needs: KN-716 wanted an ordinary navigation where focus is LOST, and drove it by clicking the navigation, but `userEvent.click` focuses what it clicks, so the reader never lost focus and the case could not occur by that route. The story failed on its own premise, `expected <button> to be <body>`, which is the whole reason a fixture should assert its premise and not only its claim: without that line it would have passed while never exercising the guard it existed to test. Driving the same navigation with Back, which activates no control, is what reaches the case.
...
- Every story sits inside the preview's `AppProviders`, whose own session provider keys the board: the board in every story keeps the bare key, and a sign-in arriving from storage remounts everything a story decorates, so a story's provider starts fresh. Which key the board reads, and a provider's state across a sign-in, are proved end to end, where the wiring is the app's; two plants passed every story before that was seen (KN-419). **That remount is the APPLICATION's behaviour and not a story's quirk**, and reading this line as a story-only caveat cost a whole iteration: `AppProviders` renders `OwnBoard`, which is `<RecordsProvider key={session?.phone ?? ''}>`, and `main.tsx` renders `App` inside it, so ANY change of reader changes that key and unmounts `App`, `Shell`, and every ref they hold. No state kept in the shell can go stale across a sign-out, in the product or in a story. The only difference is where the remount lands: a story seeds its session in its own provider BELOW `OwnBoard`, so `owner` stays `''` and the remount comes at the sign-in, while the app has one provider above it, so `owner` goes to `''` and the remount comes at the sign-out, earlier still. KN-721 said the shell's delete-confirmation latch could survive a sign-out; it was filed from a roast, taken, planned, reviewed twice, built and only then measured, and dropped as describing a state the tree cannot reach (KN-721).
...
- `todo add --parent-task` prints its flattening line FIRST, "KN-535 is itself a child of KN-431, so this hangs off KN-431", so the first id in its output is the parent's, not the new card's: read the id from the `Added KN-654:` line. A script that took the first id recorded KN-535's roast round as having filed KN-431 (KN-654).
- A Docs page draws story-docs through the markdown-to-jsx that `@storybook/addon-docs` bundles into `dist/blocks.js`, not a CommonMark parser: it ends a fence at the first place the opener's run appears after the opening line, mid-line or indented four included, and opens one on a backtick info string holding a backtick. `blocks.js` reads `document` as it loads, so a Node check cuts the bundle's `markdown-to-jsx/dist/index.modern.js` section out, hands it React, and calls the compiler the component calls, with the `forceBlock` the Markdown block passes (KN-524).
- `en-US.ts` maps every id to itself, 239 entries with no exception, and NOTHING checks that: `i18n/catalog.test.ts` reads which ids are used, whether both catalogs hold them, whether a Persian message is blank, and whether a Persian message is still its id. It never looks at an English message's value. So renaming an id and leaving its English message behind passes all 484 cases while an English reader goes on seeing the old words, which is exactly what a first attempt at KN-565 did. A rename moves the key AND the value, and a script that replaces one occurrence per pair renames the key alone, since the string is there twice (KN-663).
- A story's globals seed at the preview iframe's FIRST load: navigating an already-booted iframe to a URL whose `globals` differ changes nothing, encoded or not, and a story's own `globals` outrank the URL's in any case. Open a fresh tab to change one. Where the copy lives in a story's `args` rather than in a global, as `Shared/EmptyState` does, no global can change the language at all and only the story written for that language draws it (KN-565's look).
- A backgrounded command's "completed (exit code 0)" is the SHELL LINE's status, not the runner's: `npx vitest … > out.txt 2>&1; echo "exit $?"` always ends 0, because the `echo` succeeds. Read the summary line out of the file instead. A run reported as green on that basis had one failing story, and the failure was only found by reading the file (KN-591's build).
- Storybook runs a story at its DECLARED globals, so threading a locale or a colour scheme through a helper changes what the helper ASSERTS and not what the browser renders. A story that must be seen in another language or scheme pins it itself: that is why `App/SignIn` has English twins of its frame stories, KN-589, and two dark stories of its own, KN-591. A helper parameter with no story pinning it proves a token, never a rendered line.
- A story that derives its expected copy from the same catalog the screen renders from proves that the right LOCALE was chosen, never that the words are right: change the catalog's value and both move together. The Persian side is protected where a test hardcodes the Persian, which is also why an English-id search misses those; English has no equivalent, and `catalog.test.ts` never compares an English message with its id (KN-663, and KN-589's roast).
- **Absolute coverage language attached to a partial mechanism is the sentence that keeps being wrong here**: "only", "every", "actually", "does not cover". Three in one day, all in comments beside a check that did less than the comment said — KN-667, the body assertion that measured one direction; KN-669, the exclusion that called a live exception settled; and the repair for KN-669, which said a token "is checked against the fills it actually sits on" where that check runs in dark alone. Before committing such a sentence, trace it through the exact iterable, branch, palette or scheme, and component use sites, then ask **which concrete case would pass while violating the sentence**. If one exists, the sentence is wrong, not merely loose: a comment that overstates its check is worse than none, because the next reader trusts it instead of adding what is missing (KN-669's roast).
- **The runner names a story's test by its DISPLAY name, spaced, not by its export**: `ReportsABlankTitle` runs as `Reports A Blank Title`. So `vitest -t ReportsABlankTitle` matches nothing, and vitest then reports `Tests 6 skipped (6)` and exits 0 — an absence wearing a pass's clothes, and the same mistake as reading a backgrounded shell line's status. A run that skipped everything or ran none is a failure to run, not a run that passed, so read the summary line and refuse it on "skipped" or a zero total. KN-626's control got this wrong twice from the one wrong name: the filter matched no test, and then the script's own regex missed a real failure that the summary line was reporting two lines above it.
- **A negative assertion made after an interaction can pass because the interaction never happened.** "Press the trigger, then find no dialog" is also exactly what an unpressed trigger leaves behind, and no `waitFor` mends it, because an absence passes on its first poll. Assert the contract itself instead: hand the component the state that must be refused and watch it refuse. That is the form that fails against the code as it was — KN-626's story, told to open with a blank title, fails on the report that was never made, which a press could never have shown (KN-626's plan review, round two).
- **`locator.check()` CLICKS, even where the context has touch**, so a phone test that checks a box is driving the desktop's gesture at a phone's viewport. Read in `playwright-core`'s own bundle rather than taken on trust: `_setChecked` calls `this._click(...)` and then throws "Clicking the checkbox did not change its state". `locator.tap()` is the touch path and wants a context with `hasTouch`, which the mobile project sets. **The trap is that checking passes anyway**: KN-651's spec went 6 of 6 across both projects while its mobile branch still checked, so the green run could not tell which gesture it had sent, and only reading the call could. Playwright 1.62 has no long press at all — `Touchscreen` emulates a whole tap and `androidDevice.longTap()` is for a connected device — so a held press is CDP `Input.dispatchTouchEvent`, `touchStart`, a wait on what the hold produces, then `touchEnd`, as `board.spec.ts` and now `network.spec.ts` hold one (KN-651's plan review).
- **What the story-docs guard checks, and what it cannot.** `guard.test.ts` holds twelve tests and every one of them is STRUCTURAL: the story files are found at all; the list matches Storybook's own indexer; every title maps to exactly one file name; every story has an entry in BOTH languages; documented props match what `react-docgen` finds, in both directions, but ONLY for a story whose meta names a plain-identifier component — one with no component at all is skipped as legitimate, and one whose component is present but unreadable FAILS rather than skipping, which the file records was once the other way round and so rewarded being hard to read; every callback THE PARSER CAN STATICALLY READ is `storybook/test`'s `fn()` — one inside an args spread, one in args written as a call or a member expression, and one a `render` replaces are all out of its reach and pass unchecked, as the comment above `callbackProblems` says; every exported story has an entry and every entry names a real story; the Persian side carries the same prop and story KEYS and a non-empty description; a computed title is refused; every markdown file parses in the story-docs format; and no file is orphaned. **Not one of them reads what an entry SAYS.** A green guard means the entries exist, parse, and are named right; it is silent on whether a sentence is true, and it would pass a sentence and its exact opposite identically. Prose is checked only by reading the story it describes. Do not cite this guard as proof that a docs claim is right — KN-675's closing evidence did, which is KN-679.
- **"Nothing checks X" is a claim about the WHOLE repository, and one file's green run cannot support it.** KN-663 was filed saying nothing compares an English message with its id. `i18n.test.ts` has asserted exactly that over every entry since 2026-09-08, `e3150cc`, eight days before the card. The mistake was running `catalog.test.ts` alone, watching its 484 cases pass, and concluding the tree checks nothing: **a partial run tells you what it ran, never what exists.** Before filing a card that says nothing checks a thing, grep the tree for the check — it costs one command, and this cost a card, a plan and a review round before the reviewer found the test that was already there.
- **Fold an assertion into an existing helper when what it asserts is that helper's own SETUP; keep it out when it is a second concern.** Two cards drew this line in opposite directions on the same day and both were right. KN-668 kept a contrast assertion OUT of `codeAsTheFramesIn`, because how readable a notice is has nothing to do with measuring a card against its design frames — folding it in would have smuggled a passenger into a geometry check. KN-672 folded a status-region assertion INTO `laidOutAsTheFrames`, because that helper presses Load sample data precisely so the board has records to measure, so checking the press worked is the helper verifying its own precondition. **I reached the wrong answer in KN-672 by arguing from KN-668 by analogy**, and the review corrected it: the question is not "is this helper's subject different from my claim", it is "does my claim concern something this helper already depends on". When it does, a separate story is a second boot proving what the first one needed anyway.
- **Naming a value in a card is what puts it in the repository.** `todo render` writes every card's description into `agent/TODO_BOARD.md`, and `.claude/todo.db` is committed, so a card proposing literal replacement values for a requirement that a value appear nowhere else **disqualifies those values by proposing them**. Measured for KN-685: all three of its named candidates were already in the rendered board, one at line 11830, at the moment the card was filed. The general form is wider than the board, since a value written into a plan, an evidence line, `STATE.md` or a commit message is in the repository from then on, and so is one printed by a verifier whose output gets pasted into any of those. Where a card turns on a value being unique, choose it somewhere that is not the repository, write it only into the file that must hold it, and record the property rather than the value, so the claim stays true and stays re-checkable by re-running the search.
- **"The values I named are disqualified" is not "no value can qualify."** KN-685's first plan proved the card's three candidates were already in the board, concluded the exit condition was unsatisfiable, and proposed amending it. The review's correction: a fresh, unmentioned pair satisfies it exactly, and the plan had generalised from the instances in front of it to the whole class. This is the mirror of the overclaim this section already records seven times, the same failure to trace a sentence through its actual quantifier, reached by under-claiming instead, and its cost is worse: an overclaim ships a weak check, while a false impossibility rewrites the requirement. **Before calling an exit condition impossible, name the class the counter-examples came from and ask whether the whole class is ruled out or only the instances you happen to be holding.** Declaring a requirement impossible is a heavier claim than failing to meet it, and rule zero's "do not invent gates" gives no cover for the opposite move, quietly lowering one.
- **`git ls-files -z --cached --others --exclude-standard` is the honest search set for a whole-repository claim.** Tracked files plus non-ignored untracked ones: exact, excluding `node_modules` and build output by construction, and not dependent on listing directories correctly, which is the mistake KN-685 exists for, where candidates were checked under `src` alone and a two-language claim written on a one-language, one-directory search. **`--others` is not optional**, and KN-685 proved why with its own plan: `git ls-files` alone lists tracked files, the file likeliest to hold a value by accident is a plan or evidence draft written during that very task, and such a file is untracked while the search runs. Widening the set took the Persian value from 8 holders to 9, the ninth being the plan claiming it was unique. Compare as UTF-8 bytes, the rule `bundle-scan.mjs` already states, so a decode of a binary file can neither invent nor miss a match. This is the method the KN-663 lesson asks for: "nothing checks X" and "this value is held only here" are both whole-repository claims, and one command settles them.
- **A STORY IS NOT A CALLER, and "only its own tests use it" is the finding, not a footnote.** The owner's rule of 2026-09-16, after five cards closed correctly inside code no reader could reach: KN-314, KN-380, KN-690 and the planning of KN-689 and KN-692 all refined the Search Bar's `onSearch` and its 300 ms wait, while **no screen passed the prop at all**. The board and the contacts page filter on every keystroke through `onChange`. **Before filing or accepting behaviour work, name four things: the user action, the production screen, the prop or callback path, and the observable product effect.** If only stories reach it, it is component behaviour or a proposal, and must not be written as a claim about the product. **And a roast answers "what reaches this code?" BEFORE "is this right?"**; "only its own stories" outranks every other finding in the round. What makes this invisible is our own rules: every callback prop gets an `fn()` so the Actions panel records it, so every prop has a caller the moment it exists, and coverage and the story-docs guard are both satisfied by a prop nothing real passes. **The e2e suite is the only place that can tell the difference**, which is why the owner's other half of the rule is that a feature needs a test through the real app and not only a unit or a story: `apps/web/e2e/board.spec.ts` searches the real board, asserts it narrows, and has passed all along without the debounce ever running.
- **A test that passes either way is worth less than no test**, and it is how the above hid. That same e2e search uses `fill()`, which sets the whole value in one shot rather than a key at a time, and Playwright's assertions auto-retry until their timeout, so it would go green with or without a debounce and could never fail on its absence. Where a test exists for timing, it has to type key by key, assert the state that should NOT have arrived yet, and only then the one that should.
- **An identity assertion needs a non-emptiness assertion beside it.** "These two values are the same" is satisfied by two values that are both absent, so it passes hardest at exactly the moment the thing it guards has been deleted. KN-691's story asserts the Search Bar's hint is unchanged by passing a label, by comparing a bare bar with a labelled one, and the plan review added the clause I had missed: the bare bar's hint must be NON-EMPTY, because a regression removing both hints leaves two empty strings that are still identical and the story goes green having proved nothing. Same family as an absence passing on its first poll, and as a filtered run reporting "skipped" and exiting 0: **a check whose subject has vanished should fail, not pass.** Whenever a test compares two things rather than naming a value, ask what it does when both are gone.
- **A card's exit naming N sites is a starting point, not an inventory.** KN-690's exit named three places saying the clear searches "at once". Searching the component's folder and both story-docs files for the phrase, and its Persian, found FIVE false sentences across FOUR files, and the fourth was the `Clearing` story's own comment contradicting a correct comment **four lines below it** in the same file, written by KN-380 when it moved the call into an effect and did not look up. Fixing the three the card named would have reproduced the defect the card exists for. The card is written by someone who looked once: **search the scope yourself before believing its list is complete**, and when the search finds more, say so in the evidence rather than quietly fixing the extra.
- **A truncated page read is not a page read.** `get_page_text` stops at `max_chars` and says so, and both my first Docs-page reads stopped before the entry I had just added, while I was about to write that I had read the page in both languages. "I read it" has to mean the part being claimed about was in the output. Where a read truncates, go back for the missing part, `find` for a single element, and say which half was confirmed how. The same applies to any tool with a limit: a `head_limit` on a search, a log tail, a diff cut short.
- **When correcting prose, the true sentences using the same words are the hazard.** KN-690 replaced five sentences saying the clear searches "at once" while leaving two saying typing is reported "at once", because `onChange` genuinely is synchronous in the handler. The word was right four lines from where it was wrong, in the same paragraph, so a blanket replace would have broken the true ones and a search trusting the phrase alone would have reported failure where there was none. Read every hit and classify it, then have the closing check assert the COUNT of legitimate survivors rather than zero, so the next person meets the distinction instead of a bare number.
- **A build's sourcemap is not the record of what the build read.** `sources` and `sourcesContent` witness source text represented in emitted mapped chunks and nothing more, while Vite also copies `public/`, inlines small assets and emits generated bytes, none of which appear as a `sources` entry. Calling it the build's input graph is absolute coverage language on a partial mechanism once again. Where the question is what an artefact contains, scan the artefact: `scanBundle` already reads every regular file under `dist`, HTML and assets included, so a separate served-pages scan adds nothing to it.
- **A line that prints a verdict unconditionally is not a verdict, and it is the shell-line lesson one level up.** A background check for KN-698 ran `tsc --noEmit` and `eslint .` and then printed `eslint clean` from a bare `echo` that could not know: eslint had reported five errors and exited non-zero two lines above it, and the word "clean" was believed for a whole step. Same shape as `drift && git commit`, where the drift script prints its verdict and exits 0 either way, and same shape as reading a backgrounded command's shell status. **Print the code beside the claim, `cmd; echo "NAME_EXIT=$?"`, and read the number rather than the sentence next to it.** A script that both performs a check and narrates it must derive the narration from the result, or it is writing fiction on the check's authority.
- **A substring search cannot tell an export from a eulogy.** KN-698's close guard refused to commit the very work it was written to guard: it searched `routes.ts` for `destinationIn` and `addressOf`, and found them, in the doc comment that says they are GONE. A deletion is normally explained where it happened, so the names of deleted things live on in prose at exactly the place a naive check looks for them. **Strip comments before searching source for a name, or match a declaration rather than the bare word.** This is the mirror of "a tool that finds nothing has not proved anything": a tool that finds something has proved nothing either, until you know what it found.
- **A plan that inventories existing behaviour is a checklist for the END of the build, not only its start.** KN-698's plan recorded at line 30 that the hand-rolled shell called `history.pushState` unless the path was already current. The rewrite handed every address to react-router, which pushes whatever it is given, and that guard was never carried over: clicking the page already shown added an entry, so Back would return to the same page once for every time the reader pressed it. Nothing in the build noticed, because the plan had been read for what to DO and never re-read for what to KEEP. **Before closing, walk the plan's account of the old behaviour line by line and say where each item lives now.** The rewritten assertion caught it, four entries where it expected three; the spy it replaced could not have, since a router calls its own history implementation. Same family as KN-695, where the plan predicted the failure and my conclusion overrode it: there the warning was argued away, here it was simply never looked at again.
- **Deleting a helper can expose literals to a lint rule that the helper's TYPE was hiding.** `addressOf(destination, base)` took a `Destination`, a union of string literals, and `eslint-plugin-lingui` runs with `useTsTypes`, which skips a literal typed as such a union. Removing it turned exempt arguments into five bare strings the rule had never seen, none of them copy: they are addresses. **The fix for a false positive of that rule is a TYPE, not an exemption.** Adding `path` to `structuralProps` would have exempted the NAME in every file, which is exactly the hole KN-095 closed for `title` and KN-087 closed twice before it. `eslint.config.js` carries that history in its own comments and is worth reading before widening anything: several of its entries are epitaphs for exemptions that turned out to be holes.
- **A failure set that MOVES between identical runs is a flake; a set that holds still is a break.** Closing KN-698, the unit project failed five tests over four files, and the first instinct, that the change had broken something far away, was wrong twice over. Two were `session.test.ts`, whose card KN-551 records the identical messages. The others changed identity between two runs of the same tree: Input `Multiline` and JobCard `Pressed` in one, those plus Input `Latin In A Persian Page` in the next. **Run it again before attributing it, and attribute only what holds still.** Then keep the message: KN-551 lost its own twice to a filtered run and spent two cards re-catching it, so a run whose failures matter is written whole to a file rather than piped through `tail`.
- **An argument list has a size limit, and the command that dies on it reports success from the shell line.** KN-697's roast was launched with the evidence (6,233 bytes), the whole diff of seven files (23,696) and four long questions as inline arguments; the runner answered `Argument list too long` while the task notification said "completed (exit code 0)". That is the backgrounded-exit-code lesson one level out: the pipeline's last command succeeded, so the line was 0. **Measure the pieces before sending them** — `wc -c` on each — and cut what the reader does not need: `--files` already named the two spec files, so their 8,773 bytes of diff were pure payload. **A roast that did not run is a step of the loop not taken**, not a roast with nothing to say, and the card had already closed by then.
- **An end-to-end test proves behaviour and covers nothing.** Coverage here is counted by the unit and storybook projects; Playwright drives a built bundle and feeds no source counter. So a line can be exercised by a reader in a real browser, asserted on both viewports, and still sit uncovered against a 100 percent threshold. KN-697 met this exactly: `App.tsx`'s add-destination arm was proved by an e2e that clicks the add tab, while no STORY had ever taken that path, and the fix was to make `App.stories.tsx`'s `Navigating` press it rather than only push history. **When a change adds a branch, ask which story or unit test reaches it**, never which e2e does.
- **"No threshold error appeared" is not "coverage passed".** Vitest does not print the coverage report at all when the run fails, so a red suite hides the verdict rather than reporting it as bad. KN-697's first coverage read found only "Coverage enabled with v8", no table, and established nothing either way. `--coverage.reportOnFailure=true` emits it anyway; and `coverage/lcov.info` is already on disk from the previous run, so per-file uncovered lines and branches can be read with no re-run at all — `DA:<line>,0` for lines and `BRDA:<line>,…,0` for branches.
- **A branch reachable only through a timing window cannot be covered by a story, so move the decision into a function.** KN-697's search step guarded the case where a settled search changes nothing, reachable only by typing a letter and deleting it inside the Search Bar's 300 ms pause. A story racing that pause would cover the branch by luck, which is a COVERAGE flake and worse than a test flake, since it fails a gate for a reason no one can reproduce. Moved into `searchStep` in `routes.ts`, every case is exact, `routes.test.ts` proves six with no clock, the screens are left with no branch of their own, and the duplication between the two screens went with it. **Measured, not feared**: it had shown up uncovered at `JobsScreen.tsx:120` and `NetworkScreen.tsx:93` first.
- **Uncovered line numbers move when lines are removed, so compare the SET and not the numbers.** After KN-697's refactor `JobsScreen.tsx` reported 88, 230, 238, 253, 275, 624, 755 where it had reported 88, 120, 235, 243, 258, 280, 629, 760. Read carelessly that is six new gaps; read properly it is the same set minus 120, shifted down by the five lines the refactor deleted. **Before calling an uncovered line new, subtract the change in file length, or open the line and read it.**
- **`prefer-as-const` and this repo's ban on a bare literal `as const` meet at a typed constant, and a named type satisfies both.** KN-217 forbids `const X = 'q' as const`, because a bare literal written that way is skipped by `no-unlocalized-strings` before any other check; `@typescript-eslint/prefer-as-const` forbids `const X: 'q' = 'q'`. They do not actually contradict: `type QueryName = 'q'` and then `const QUERY: QueryName = 'q'` passes both, and `useTsTypes` still skips the literal because its type is a union of string literals. The repo's own rule message names that way out — "type the binding against a union" — so read the message before treating a pair of rules as a deadlock.
