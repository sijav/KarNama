...
describe('story-docs guard', () => {
  ...
  it('every markdown file fits the format, with nothing the parser had to leave out', () => {
    // KN-202, KN-405 and KN-524. The parser reports what does not fit rather
    // than absorbing it: a section other than Props and Stories, a second entry
    // of one name, an entry or text outside every entry, a heading of level one
    // or of four to six, a fence that never closes, the first line in a fence
    // that a Docs page takes for its end, and an entry with no prose. Here each
    // is a failure naming the file.
    let read = 0
    for (const language of ['en', 'fa'] as const) {
      for (const path of globSync('*.md', { cwd: join(DOCS, language) })) {
        const { problems } = parseStoryDoc(readFileSync(join(DOCS, language, path), 'utf8'))
        read += 1
        expect
          .soft(
            problems.map(({ message }) => message),
            `${language}/${basename(path)} does not fit the story-docs format`,
          )
          .toEqual([])
      }
    }
    // The positive control: no files read passes the loop above identically.
    expect(read).toBeGreaterThan(files.length)
  })
