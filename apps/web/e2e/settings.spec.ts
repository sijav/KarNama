...
test('settings apply immediately and survive reload; sample loading does not duplicate records', async ({ page }, testInfo) => {
  ...
  // The language is a Select, KN-480. Its list is portalled to the page, outside
  // the dialog, so the option is found on the page.
  await dialog.getByRole('combobox', { name: 'زبان' }).click()
  await page.getByRole('option', { name: 'English', exact: true }).click()
  await expect(page.getByRole('listbox')).toHaveCount(0)
  await expect(page.locator('html')).toHaveAttribute('lang', 'en-US')
  await expect(page.locator('html')).toHaveAttribute('dir', 'ltr')
  await expect(dialog.getByRole('combobox', { name: 'Language' })).toBeFocused()
  await expect(dialog.getByRole('combobox', { name: 'Language' })).toHaveText('English')
