...
for (const locale of ['fa-IR', 'en-US']) {
  for (const mode of ['light', 'dark']) {
    test(`cards drag between statuses and persist in ${locale} ${mode}`, async ({ page }, testInfo) => {
      ...
      await saved.getByRole('button', { expanded: true }).click()
      await expect(saved.getByRole('article')).toHaveCount(0)
      const savedToggle = saved.getByRole('button', { expanded: false })
      await savedToggle.focus()
      await page.keyboard.press('Enter')
      await expect(saved.getByRole('article')).toHaveCount(1)
      await rejected.getByRole('button', { expanded: false }).click()
      await rejected.getByRole('button', { expanded: true }).click()
      await expect(rejected.getByRole('button', { expanded: false })).toBeVisible()
      ...
      await rejected.getByRole('button', { expanded: true }).click()
      await expect(rejected.getByRole('article')).toHaveCount(0)
      await rejected.getByRole('button', { expanded: false }).click()
      await expect(rejected.getByRole('button', { name: 'Drag scenario', exact: true })).toBeVisible()
